...
**指令**: 确保系统安装了 Node.js v20.11.0，并配置 pnpm v8.15.0 作为包管理器。
**验证标准**: 
- Given: 终端环境
- When: 执行 `node -v` 和 `pnpm -v`
- Then: 分别显示 `v20.11.0` 和 `8.15.0`
...
#### 任务 3.1.3: 实现项目详情页
**指令**: 创建项目详情视图，展示指定项目的详细配置、构建状态和部署信息。
**验证标准**:
- Given: 一个项目 ID。
- When: 点击项目列表中的项目卡片，导航到详情页。
- Then: 页面显示该项目的完整信息和可执行的操作按钮。
