...
export const createConfigCommand = () => {
  ...
  configCommand
    ...
    .argument(
      '<key>',
      'Configuration key in dot notation (e.g., brand.personal.name)'
    )
  ...
  configCommand
    ...
    .option(
      '-s, --step <step>',
      'Run specific configuration step (personal, social, visual, defaults, review)'
    )
      ...
      try {
        ...
        if (options.step) {
          ...
          if (!validSteps.includes(options.step as WizardStep)) {
            logger.error(
              `Invalid step: ${options.step}. Valid steps are: ${validSteps.join(', ')}`
            );
            process.exit(1);
          }
          ...
          logger.success(
            `✅ Brand configuration step '${options.step}' completed!`
          );
        } else {
          // 运行完整向导
          await runBrandWizard(wizardOptions);
          logger.success(
            '🎉 Brand configuration wizard completed successfully!'
          );
        }
        ...
        logger.info(
          '💡 Tip: Run `astro-launcher config get brand` to view your current brand configuration.'
        );
      } catch (error) {
        if (
